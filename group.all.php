 <?php
	...
	$getplace = mysql_query("SELECT `artist` FROM `last_fm_charts_all` ORDER BY playcount DESC "); 
	$i=0;
	...
	foreach($places as $artist_name){
		$getartist = mysql_query("SELECT `playcount` FROM `last_fm_charts_all` WHERE artist LIKE '$artist_name'"); 
		$artist =	mysql_fetch_row($getartist);
		$count=$artist[0];
		if($place==1) {$count_max=$count;}
		$getuser = mysql_query("SELECT `user` FROM `last_fm_charts_all` WHERE artist LIKE '$artist_name'"); 
		...
		echo'
			<tr class="" style="'; if($i==0) { echo'background-color: #F2F2F2;';} echo'">
    			...
    			<td class="list" style="padding-right:5px; padding-left:8px; min-width:260px;">
      	   ...
      	   <td class="list" style="padding-right:8px; min-width:220px;"><div class="'; if($st>strlen($count)*2){ echo'textunter';} echo '">
    				'; 
    					while($m<$st) {
